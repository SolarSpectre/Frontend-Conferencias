...
const EditarAuditorio = () => {
  ...
  useEffect(() => {
    const fetchAuditorio = async () => {
      try {
        const response = await axiosInstance.get(`/auditorium/${id}`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
        setAuditorio(response.data);
      } catch (error) {
        console.error("Error al obtener el auditorio:", error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axiosInstance.put(`/auditorium/${id}`, auditorio, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  return (
    <div className="p-8">
      <h1 className="font-bold text-2xl mb-6">Editar Auditorio</h1>
